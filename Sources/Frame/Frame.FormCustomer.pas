...
uses
  ...
  FGX.SearchEdit, FGX.Button.Types, FGX.Button, FGX.Popup;

type
  TCustomerItemType = (ltCustomerData, ltHeader, ltBalance, ltHouseInfo,
    ltEquipment, ltService, ltPhone, ltNotice, ltParentEquipment);
...
type
  ...
  public
    ...
    constructor Create(AIsHeader: Boolean; ATextPrimary, ATextSecondary,
      AIcon: string; AId: integer; AItemType: integer;
      AKind: TCustomerItemType);
  end;
...
type
  TmwFormCustomer = class(TmwBaseListViewForm)
    ...
    procedure lvContentBindItem(Sender: TObject; const AIndex: integer;
      const AStyle: string; const AItem: TfgItemWrapper);
    function lvContentGetItemStyle(Sender: TObject;
...
begin
  ...
  {
    if Self.ThemeName = 'Theme Dark' then
    IcoRight := R.Bitmap.THEME_DARK_ICONS_CHEVRON_RIGHT
    else
    IcoRight := R.Bitmap.THEME_LIGHT_ICONS_CHEVRON_RIGHT;
  }

  BookmarkUpdate;
  nbForm.Subtitle := Format('Лицевой: %s', [FCustomer.account]);
  FCurrentList.Clear;
  item := TCustomerItem.Create(false, String.Join(' ', [FCustomer.firstname,
    FCustomer.midlename, FCustomer.surname]), '', '', -1, -1,
    TCustomerItemType.ltCustomerData);
  FCurrentList.Add(item);
  item := TCustomerItem.Create(true, 'Баланс', '', '', -1, -1,
    TCustomerItemType.ltHeader);
  FCurrentList.Add(item);
  item := TCustomerItem.Create(false, FCustomer.balance, '', '', -1, -1,
    TCustomerItemType.ltBalance);
  FCurrentList.Add(item);
  if not FCustomer.phones.IsEmpty then
  begin
    item := TCustomerItem.Create(true, 'Позвонить', '', '', -1, -1,
      TCustomerItemType.ltHeader);
    FCurrentList.Add(item);
    aPhones := PhonesSplit(FCustomer.phones);
    for i := Low(aPhones) to High(aPhones) do
    begin
      item := TCustomerItem.Create(false, aPhones[i], '', R.Bitmap.ICONS_PHONE,
        -1, -1, TCustomerItemType.ltPhone);
      FCurrentList.Add(item)
    end
  end;
  ...
  begin
    item := TCustomerItem.Create(true, 'Примечание', '', '', -1, -1,
      TCustomerItemType.ltHeader);
    FCurrentList.Add(item);
    item := TCustomerItem.Create(false, FCustomer.notice, '', '', -1, -1,
      TCustomerItemType.ltNotice);
    FCurrentList.Add(item);
  end;
  item := TCustomerItem.Create(true, 'Адрес', '', '', -1, -1,
    TCustomerItemType.ltHeader);
  FCurrentList.Add(item);
  item := TCustomerItem.Create(false, String.Join(', ', [FCustomer.street,
    FCustomer.house, FCustomer.flat]), '', IcoRight, FCustomer.house_id, -1,
    TCustomerItemType.ltHouseInfo);
  FCurrentList.Add(item);

  if FCustomer.services.Count > 0 then
  begin
    item := TCustomerItem.Create(true, 'Подключенные услуги', '', '', -1, -1,
      TCustomerItemType.ltHeader);
    FCurrentList.Add(item);
    for i := 0 to FCustomer.services.Count - 1 do
    begin
      item := TCustomerItem.Create(false, FCustomer.services[i].name,
        FCustomer.services[i].tarif, // ???
  ...
  begin
    item := TCustomerItem.Create(true, 'Оборудование', '', '', -1, -1,
      TCustomerItemType.ltHeader);
    FCurrentList.Add(item);
    curParentId := -1;
    ...
    begin
      ...
      begin
        item := TCustomerItem.Create(false, 'Подключен к:',
          FCustomer.equipment[i].parent_name, IcoRight,
          FCustomer.equipment[i].parent_id, -1,
          TCustomerItemType.ltParentEquipment);
        FCurrentList.Add(item);
        curParentId := FCustomer.equipment[i].parent_id;
      end;
      item := TCustomerItem.Create(false,
        IfThen(FCustomer.equipment[i].port.IsEmpty, FCustomer.equipment[i].name,
        Format('[ %s ] %s', [FCustomer.equipment[i].port,
        FCustomer.equipment[i].name])), FCustomer.equipment[i].mac, IcoRight,
        FCustomer.equipment[i].Id, FCustomer.equipment[i].e_type.ToInteger,
...
begin
  s := Self.ClassName;
  nbForm.ActionButtons.ByName['bnBookmark'].Tag := BookMarks.ContainsBookMark(s,
    FObjectId);
  if nbForm.ActionButtons.ByName['bnBookmark'].Tag = -1 then
    nbForm.ActionButtons.ByName['bnBookmark'].IconName :=
      R.Bitmap.ICONS_BOOKMARK_BORDER
  else
    nbForm.ActionButtons.ByName['bnBookmark'].IconName :=
      R.Bitmap.ICONS_BOOKMARK
end;

procedure TmwFormCustomer.DoAction(const ACard: TfgCardPanel;
  const Id, eType: integer);
{
  var
  API: TmwAPI;
  aJson: string;
...
procedure TmwFormCustomer.lvContentBindItem(Sender: TObject;
...
begin
    ...
    begin
      if FCustomer.balance.ToSingle < 0 then
        AItem.GetControlByLookupName<TfgCardPanel>('card').BackgroundColorName
          := R.Color.COLORS_CUSTOMERPAYMENTSOUT
      else
        AItem.GetControlByLookupName<TfgCardPanel>('card').BackgroundColorName
          := R.Color.COLORS_CUSTOMEROFF
    end;
  end;
  ...
  begin
    ...
    NewHeight := AItem.GetControlByLookupName<TfgLabel>('primary')
      .MeasureSize(TfgMeasuringSpecification.Fixed, Width - 16,
      TfgMeasuringSpecification.Unspecified, 0).Height +
      AItem.GetControlByLookupName<TfgLabel>('secondary').Size.Height + 8;
    AItem.item.Height := NewHeight;
    AItem.item.Realign;
  end;
  if (AStyle <> 'Style-Header') and (AStyle <> 'Style-Notice') then
    AItem.GetControlByLookupName<TfgImage>('icon').ImageName :=
      FCurrentList[AIndex].Icon;
  if AStyle = 'Style-Notice' then
  begin
    NewHeight := AItem.GetControlByLookupName<TfgLabel>('primary')
      .MeasureSize(TfgMeasuringSpecification.Fixed, Width - 16,
      TfgMeasuringSpecification.Unspecified, 0).Height + 16;
    AItem.item.Height := NewHeight;
    AItem.item.Realign;
  end;
end;
...
function TmwFormCustomer.lvContentGetItemStyle(Sender: TObject;
  const AIndex: integer): string;
begin
  inherited;
  case FCurrentList[AIndex].Kind of
    TCustomerItemType.ltEquipment, TCustomerItemType.ltParentEquipment,
      TCustomerItemType.ltService:
...
procedure TmwFormCustomer.lvContentTapItem(Sender: TObject;
...
{$ENDIF}
    ...
    TCustomerItemType.ltHouseInfo:
      begin
        CloseMe(Self, true);
        Formmain.RunFrame(TmwFormCustomers, [FCurrentList[AIndex].Id],
          'Абоненты', FCurrentList[AIndex].TextPrimary);
        Formmain.AddToStack(TmwFormCustomer, Name, [FObjectId, AIndex], title,
          Subtitle);
      end;
    TCustomerItemType.ltEquipment:
      begin
        if lvContent.FindItemWrapperByIndex(AIndex, AItem) then begin
    ...
    TCustomerItemType.ltParentEquipment:
      begin
        CloseMe(Self, true);
        Formmain.RunFrame(TmwFormEquipment, [FCurrentList[AIndex].Id,
          FCurrentList[AIndex].ItemType], 'Оборудование',
          FCurrentList[AIndex].TextSecondary);
        Formmain.AddToStack(TmwFormCustomer, Name, [FObjectId, AIndex], title,
          Subtitle);
      end;
...
constructor TCustomerItem.Create(AIsHeader: Boolean;
  ATextPrimary, ATextSecondary, AIcon: string; AId: integer; AItemType: integer;
  AKind: TCustomerItemType);
begin
  isHeader := AIsHeader;
